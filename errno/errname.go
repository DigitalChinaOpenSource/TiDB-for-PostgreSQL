...
import "github.com/DigitalChinaOpenSource/DCParser/mysql"
...
var MySQLErrName = map[uint16]*mysql.ErrMessage{
	...
	ErrFlushMasterBinlogClosed:                  mysql.Message("Binlog closed, cannot RESET MASTER", nil),
	ErrIndexRebuild:                             mysql.Message("Failed rebuilding the index of  dumped table '%-.192s'", nil),
	ErrMaster:                                   mysql.Message("Error from master: '%-.64s'", nil),
	ErrMasterNetRead:                            mysql.Message("Net error reading from master", nil),
	ErrMasterNetWrite:                           mysql.Message("Net error writing to master", nil),
	...
	ErrSlaveMustStop:                            mysql.Message("This operation cannot be performed with a running slave; run STOP SLAVE first", nil),
	ErrSlaveNotRunning:                          mysql.Message("This operation requires a running slave; configure slave and do START SLAVE", nil),
	ErrBadSlave:                                 mysql.Message("The server is not configured as slave; fix in config file or with CHANGE MASTER TO", nil),
	ErrMasterInfo:                               mysql.Message("Could not initialize master info structure; more error messages can be found in the MySQL error log", nil),
	ErrSlaveThread:                              mysql.Message("Could not create slave thread; check system resources", nil),
	...
	ErrConnectToMaster:                          mysql.Message("Error connecting to master: %-.128s", nil),
	ErrQueryOnMaster:                            mysql.Message("Error running query on master: %-.128s", nil),
	...
	ErrMasterFatalErrorReadingBinlog:            mysql.Message("Got fatal error %d from master when reading data from binary log: '%-.320s'", nil),
	ErrSlaveIgnoredTable:                        mysql.Message("Slave SQL thread ignored the query because of replicate-*-table rules", nil),
	...
	ErrSlaveWasRunning:                          mysql.Message("Slave is already running", nil),
	ErrSlaveWasNotRunning:                       mysql.Message("Slave already has been stopped", nil),
	...
	ErrSlaveIgnoredSslParams:                    mysql.Message("SSL parameters in CHANGE MASTER are ignored because this MySQL slave was compiled without SSL support; they can be used later if MySQL slave with SSL is started", nil),
	...
	ErrBadSlaveUntilCond:                        mysql.Message("Incorrect parameter or combination of parameters for START SLAVE UNTIL", nil),
	ErrMissingSkipSlave:                         mysql.Message("It is recommended to use --skip-slave-start when doing step-by-step replication with START SLAVE UNTIL; otherwise, you will get problems if you get an unexpected slave's mysqld restart", nil),
	...
	ErrBinlogRowWrongTableDef:                   mysql.Message("Table definition on master and slave does not match: %s", nil),
	ErrBinlogRowRbrToSbr:                        mysql.Message("Slave running with --log-slave-updates must use row-based binary logging to be able to replicate row-based binary log events", nil),
	...
	ErrSlaveIncident:                            mysql.Message("The incident %s occurred on the master. Message: %-.64s", nil),
	...
	ErrSlaveFatal:                               mysql.Message("Fatal : %s", nil),
	ErrSlaveRelayLogReadFailure:                 mysql.Message("Relay log read failure: %s", nil),
	ErrSlaveRelayLogWriteFailure:                mysql.Message("Relay log write failure: %s", nil),
	ErrSlaveCreateEventFailure:                  mysql.Message("Failed to create %s", nil),
	ErrSlaveMasterComFailure:                    mysql.Message("Master command %s failed: %s", nil),
	...
	ErrNeverUsed:                                mysql.Message("Ambiguous slave modes combination. %s", nil),
	...
	ErrSlaveCorruptEvent:                                     mysql.Message("Corrupted replication event was detected", nil),
	...
	WarnNoMasterInfo:                                         mysql.Message("The master info structure does not exist", nil),
	...
	ErrSlaveHeartbeatFailure:                                 mysql.Message("Unexpected master's heartbeat data: %s", nil),
	ErrSlaveHeartbeatValueOutOfRange:                         mysql.Message("The requested value for the heartbeat period is either negative or exceeds the maximum allowed (%s seconds).", nil),
	...
	ErrSlaveIgnoreServerIds:                                  mysql.Message("The requested server id %d clashes with the slave startup option --replicate-same-server-id", nil),
	...
	ErrBinlogUnsafeSystemTable:                               mysql.Message("The statement is unsafe because it uses the general log, slow query log, or performanceSchema table(s). This is unsafe because system tables may differ on slaves.", nil),
	...
	ErrBinlogUnsafeUdf:                                       mysql.Message("Statement is unsafe because it uses a UDF which may not return the same value on the slave.", nil),
	ErrBinlogUnsafeSystemVariable:                            mysql.Message("Statement is unsafe because it uses a system variable that may have a different value on the slave.", nil),
	...
	ErrSlaveConversionFailed:                                 mysql.Message("Column %d of table '%-.192s.%-.192s' cannot be converted from type '%-.32s' to type '%-.32s'", nil),
	ErrSlaveCantCreateConversion:                             mysql.Message("Can't create conversion table for table '%-.192s.%-.192s'", nil),
	...
	ErrSlaveHeartbeatValueOutOfRangeMin:                      mysql.Message("The requested value for the heartbeat period is less than 1 millisecond. The value is reset to 0, meaning that heartbeating will effectively be disabled.", nil),
	ErrSlaveHeartbeatValueOutOfRangeMax:                      mysql.Message("The requested value for the heartbeat period exceeds the value of `slaveNetTimeout' seconds. A sensible value for the period should be less than the timeout.", nil),
	...
	ErrBinlogUnsafeInsertIgnoreSelect:                        mysql.Message("INSERT IGNORE... SELECT is unsafe because the order in which rows are retrieved by the SELECT determines which (if any) rows are ignored. This order cannot be predicted and may differ on master and the slave.", nil),
	ErrBinlogUnsafeInsertSelectUpdate:                        mysql.Message("INSERT... SELECT... ON DUPLICATE KEY UPDATE is unsafe because the order in which rows are retrieved by the SELECT determines which (if any) rows are updated. This order cannot be predicted and may differ on master and the slave.", nil),
	ErrBinlogUnsafeReplaceSelect:                             mysql.Message("REPLACE... SELECT is unsafe because the order in which rows are retrieved by the SELECT determines which (if any) rows are replaced. This order cannot be predicted and may differ on master and the slave.", nil),
	ErrBinlogUnsafeCreateIgnoreSelect:                        mysql.Message("CREATE... IGNORE SELECT is unsafe because the order in which rows are retrieved by the SELECT determines which (if any) rows are ignored. This order cannot be predicted and may differ on master and the slave.", nil),
	ErrBinlogUnsafeCreateReplaceSelect:                       mysql.Message("CREATE... REPLACE SELECT is unsafe because the order in which rows are retrieved by the SELECT determines which (if any) rows are replaced. This order cannot be predicted and may differ on master and the slave.", nil),
	ErrBinlogUnsafeUpdateIgnore:                              mysql.Message("UPDATE IGNORE is unsafe because the order in which rows are updated determines which (if any) rows are ignored. This order cannot be predicted and may differ on master and the slave.", nil),
	...
	ErrBinlogUnsafeWriteAutoincSelect:                        mysql.Message("Statements writing to a table with an auto-increment column after selecting from another table are unsafe because the order in which rows are retrieved determines what (if any) rows will be written. This order cannot be predicted and may differ on master and the slave.", nil),
	ErrBinlogUnsafeCreateSelectAutoinc:                       mysql.Message("CREATE TABLE... SELECT...  on a table with an auto-increment column is unsafe because the order in which rows are retrieved by the SELECT determines which (if any) rows are inserted. This order cannot be predicted and may differ on master and the slave.", nil),
	...
	ErrMasterDelayValueOutOfRange:                            mysql.Message("The requested value %d for the master delay exceeds the maximum %d", nil),
	...
	ErrMtsFeatureIsNotSupported:                              mysql.Message("%s is not supported in multi-threaded slave mode. %s", nil),
	...
	ErrInsecureChangeMaster:                                  mysql.Message("Storing MySQL user name or password information in the master.info repository is not secure and is therefore not recommended. Please see the MySQL Manual for more about this issue and possible alternatives.", nil),
	...
	ErrSQLthreadWithSecureSlave:                              mysql.Message("Setting authentication options is not possible when only the Slave SQL Thread is being started.", nil),
	...
	ErrBadSlaveAutoPosition:                                  mysql.Message("Parameters MASTERLOGFILE, MASTERLOGPOS, RELAYLOGFILE and RELAYLOGPOS cannot be set when MASTERAUTOPOSITION is active.", nil),
	ErrAutoPositionRequiresGtidModeOn:                        mysql.Message("CHANGE MASTER TO MASTERAUTOPOSITION = 1 can only be executed when @@GLOBAL.GTIDMODE = ON.", nil),
	...
	ErrGtidModeRequiresBinlog:                                mysql.Message("@@GLOBAL.GTIDMODE = ON or UPGRADESTEP1 or UPGRADESTEP2 requires --log-bin and --log-slave-updates.", nil),
	...
	ErrMasterHasPurgedRequiredGtids:                          mysql.Message("The slave is connecting using CHANGE MASTER TO MASTERAUTOPOSITION = 1, but the master has purged binary logs containing GTIDs that the slave requires.", nil),
	...
	ErrSlaveConfiguration:                                    mysql.Message("Slave is not configured or failed to initialize properly. You must at least set --server-id to enable either a master or a slave. Additional error messages can be found in the MySQL error log.", nil),
	...
	ErrMtsChangeMasterCantRunWithGaps:                        mysql.Message("CHANGE MASTER cannot be executed when the slave was stopped with an error or killed in MTS mode. Consider using RESET SLAVE or START SLAVE UNTIL.", nil),
	ErrMtsRecoveryFailure:                                    mysql.Message("Cannot recover after SLAVE errored out in parallel execution mode. Additional error messages can be found in the MySQL error log.", nil),
	...
	ErrSlaveSilentRetryTransaction:                           mysql.Message("Slave must silently retry current transaction", nil),
	...
	ErrSQLSlaveSkipCounterNotSettableInGtidMode:              mysql.Message("sqlSlaveSkipCounter can not be set when the server is running with @@GLOBAL.GTIDMODE = ON. Instead, for each transaction that you want to skip, generate an empty transaction with the same GTID as the transaction", nil),
	...
	ErrDataTruncatedFunctionalIndex:                          mysql.Message("Data truncated for functional index '%s' at row %d", nil),
	ErrDataOutOfRangeFunctionalIndex:                         mysql.Message("Value is out of range for functional index '%s' at row %d", nil),
	ErrFunctionalIndexOnJSONOrGeometryFunction:               mysql.Message("Cannot create a functional index on a function that returns a JSON or GEOMETRY value", nil),
	ErrFunctionalIndexRefAutoIncrement:                       mysql.Message("Functional index '%s' cannot refer to an auto-increment column", nil),
	ErrCannotDropColumnFunctionalIndex:                       mysql.Message("Cannot drop column '%s' because it is used by a functional index. In order to drop the column, you must remove the functional index", nil),
	ErrFunctionalIndexPrimaryKey:                             mysql.Message("The primary key cannot be a functional index", nil),
	ErrFunctionalIndexOnLob:                                  mysql.Message("Cannot create a functional index on an expression that returns a BLOB or TEXT. Please consider using CAST", nil),
	ErrFunctionalIndexFunctionIsNotAllowed:                   mysql.Message("Expression of functional index '%s' contains a disallowed function", nil),
	ErrFulltextFunctionalIndex:                               mysql.Message("Fulltext functional index is not supported", nil),
	ErrSpatialFunctionalIndex:                                mysql.Message("Spatial functional index is not supported", nil),
	ErrWrongKeyColumnFunctionalIndex:                         mysql.Message("The used storage engine cannot index the expression '%s'", nil),
	ErrFunctionalIndexOnField:                                mysql.Message("Functional index on a column is not supported. Consider using a regular index instead", nil),
	ErrFKIncompatibleColumns:                                 mysql.Message("Referencing column '%s' in foreign key constraint '%s' are incompatible", nil),
	ErrFunctionalIndexRowValueIsNotAllowed:                   mysql.Message("Expression of functional index '%s' cannot refer to a row value", nil),
	ErrDependentByFunctionalIndex:                            mysql.Message("Column '%s' has a functional index dependency and cannot be dropped or renamed", nil),
	ErrInvalidJSONValueForFuncIndex:                          mysql.Message("Invalid JSON value for CAST for functional index '%s'", nil),
	ErrJSONValueOutOfRangeForFuncIndex:                       mysql.Message("Out of range JSON value for CAST for functional index '%s'", nil),
	ErrFunctionalIndexDataIsTooLong:                          mysql.Message("Data too long for functional index '%s'", nil),
	ErrFunctionalIndexNotApplicable:                          mysql.Message("Cannot use functional index '%s' due to type or collation conversion", nil),
	...
	ErrCantDecodeIndex:            mysql.Message("Cannot decode index value, because %s", nil),
