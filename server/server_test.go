...
func (cli *testServerClient) runTestRegression(c *C, overrider configOverriderPG, dbName string) {
	cli.runTestsOnNewDB(c, overrider, dbName, func(dbt *DBTest) {
		...
		// Update
		dbt.mustExec("UPDATE test SET val = 0 WHERE val = ?", 1)
		...
		// Delete
		dbt.mustExec("DELETE FROM test WHERE val = 0")
...
func (cli *testServerClient) runTestSpecialType(t *C) {
	cli.runTestsOnNewDB(t, nil, "SpecialType", func(dbt *DBTest) {
		...
		t.Assert(outB, Equals, "2012-12-21T12:12:12Z") // time output to ISO 8601 String
		t.Assert(outC, Equals, "0000-01-01T04:23:34Z")
...
// PG Modified
// Changed assert format into ISO 8601 String
func (cli *testServerClient) runTestPreparedTimestamp(t *C) {
	cli.runTestsOnNewDB(t, nil, "prepared_timestamp", func(dbt *DBTest) {
		...
		t.Assert(outB, Equals, "0000-01-01T23:59:59Z")
	})
}

func (cli *testServerClient) runTestLoadDataWithSelectIntoOutfile(c *C, server *Server) {
	cli.runTestsOnNewDB(c, nil, "SelectIntoOutfile", func(dbt *DBTest) {
...
func (cli *testServerClient) runTestLoadDataForSlowLog(c *C, server *Server) {
	...
	cli.runTestsOnNewDB(c, nil, "load_data_slow_query", func(dbt *DBTest) {
...
//	cli.runTestsOnNewDB(c, nil, "LoadData", func(dbt *DBTest) {
...
// PG Modified
// Currently this doesn't really test for concurrent update, we need changes in parser in order to account for
// grammatical differences in transaction between postgres and mysql
func (cli *testServerClient) runTestConcurrentUpdate(c *C) {
	...
	cli.runTestsOnNewDB(c, nil, dbName, func(dbt *DBTest) {
		...
		concurrencyHelper(dbt, dbName) //preferably this will run in a go routine, but its currently causing trouble
		concurrencyHelper(dbt, dbName)
	})
}

//A helper function to test concurrency
//PG Modified
func concurrencyHelper(dbt *DBTest, dbName string) {
	dbt.mustExec("START TRANSACTION")
	dbt.mustExec(fmt.Sprintf("USE `%s`;", dbName))
	dbt.mustExec("update test2 set a = a + 1 where b = 1")
	dbt.mustExec("COMMIT")
}
...
// checkErrorCode is a utility function that will return true if e matches any of given error codes
// It's being used by test load data
// PG Modified
func checkErrorCode(c *C, e error, codes ...string) {
	pqError, ok := e.(*pq.Error)
	c.Assert(ok, IsTrue, Commentf("err: %v", e))
	errorCodeStr := string(pqError.Code)
	if len(codes) == 1 {
		c.Assert(errorCodeStr, Equals, codes[0])
	}
	isMatchCode := false
	for _, code := range codes {
		if errorCodeStr == code {
			isMatchCode = true
			break
		}
	}
	c.Assert(isMatchCode, IsTrue, Commentf("got err %v, expected err codes %v", errorCodeStr, codes))
}
...
func (cli *testServerClient) runTestAuth(c *C) {
	...
	// We currently skip login through IP that not exists in mysql.user
...
func (cli *testServerClient) runTestIssue3662(c *C) {
	...
	err = db.Ping()
	c.Assert(err, NotNil)
	c.Assert(err.Error(), Equals, "pq: database \"non_existing_schema\" does not exist")
}
...
func (cli *testServerClient) runTestIssue3680(c *C) {
	...
	err = db.Ping()
	c.Assert(err, NotNil)
	c.Assert(err.Error(), Equals, "pq: password authentication failed for user \"non_existing_user\"")
}
...
func (cli *testServerClient) runTestIssue3682(c *C) {
	...
	err = db.Ping()
	c.Assert(err, NotNil)
	c.Assert(err.Error(), Equals, "pq: password authentication failed for user \"issue3682\"")
}
